...
| `ops/scripts/frontend-nginx-up.sh`   | 用容器 Nginx 验证前端静态资源和接口代理   | 监听 `8080`             |
...
默认接口地址是 `http://localhost:9527`。需要切换接口地址时，通过环境变量传入：

```bash
EXPO_PUBLIC_API_BASE_URL=http://localhost:9527 ./ops/scripts/desktop-tauri-build.sh
```
